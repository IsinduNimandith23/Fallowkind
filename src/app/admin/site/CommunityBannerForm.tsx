...
export default function CommunityBannerForm({ currentUrl }: { currentUrl: string }) {
  ...
  async function saveUrlToServer(nextUrl: string) {
    const res = await fetch("/api/admin/settings/community-banner", {
      method: "PUT",
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    try {
      ...
      showMessage(setSuccess, "Community banner updated.");
      router.refresh();
    } catch (e) {
  ...
  async function handleClear() {
    ...
    try {
      ...
      setPreview("");
      if (fileRef.current) fileRef.current.value = "";
      showMessage(setSuccess, "Community banner cleared.");
      router.refresh();
    } catch (e) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Current community banner</p>
          {preview && (
            <button
              type="button"
              onClick={handleClear}
        ...
        <div className="aspect-[21/9] bg-sage relative">
          {preview ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-linen/70 text-sm text-center px-4">
              No banner set - the default image will show on the community page
            </div>
          )}
        </div>
      ...
      <form onSubmit={handleUpload} className="bg-white rounded-lg border border-gray-100 shadow-sm p-6 space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-2">
            JPG, PNG, WebP, or GIF. Max 10MB. Recommended a wide image (≈ 21:9, e.g. 2400 × 1030 px). Goes to <code className="bg-gray-100 px-1 rounded">media/banner</code>.
          </p>
        </div>
        <button
          type="submit"
