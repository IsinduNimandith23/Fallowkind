...
export default function HeroVideoForm({ currentUrl }: { currentUrl: string }) {
  ...
  async function handleSaveUrl(e: React.FormEvent) {
    ...
    try {
      ...
      setPreview(url.trim());
      showMessage(setSuccess, "Hero video URL saved.");
      router.refresh();
    } catch (e) {
        ...
        <form onSubmit={handleSaveUrl} className="bg-white rounded-lg border border-gray-100 shadow-sm p-6 space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="https://… or /mp5.mp4"
              className="w-full border border-gray-200 rounded px-3 py-2.5 text-sm focus:outline-none focus:border-forest"
            />
            <p className="text-xs text-gray-400 mt-2">
              Any direct video URL (CDN, public Supabase Storage link, or a path under <code className="bg-gray-100 px-1 rounded">/public</code>).
            </p>
          </div>
          <button
            type="submit"
