...
export default function CommunityQuotes({ quotes }: { quotes: Quote[] }) {
  ...
  return (
    <div className="group relative max-h-[28rem] overflow-hidden">
      ...
      <div className="flex flex-col gap-4 animate-marquee-y group-hover:[animation-play-state:paused] motion-reduce:animate-none">
        {loop.map((q, i) => (
          <article key={`${q.author}-${i}`} className="glass-card p-5 shrink-0">
